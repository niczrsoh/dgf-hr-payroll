          ...
          <div ref={printRef} className="payslip-wrap" style={{ fontFamily: 'Arial, sans-serif', fontSize: '11px', color: '#000', maxWidth: '780px', margin: '0 auto' }}>
            ...
            <div style={{ border: '1px solid #000', borderTop: 'none', padding: '6px 10px' }}>
              <div style={{ display: 'flex', gap: '40px', marginBottom: '3px' }}>
                <span>No.Pekerja : <strong>{employee.employeeNo}</strong></span>
                <span>No.KP. : <strong>{employee.icNumber}</strong></span>
                <span>No.KWSP : <strong>{employee.epfNumber || '-'}</strong></span>
                <span>No.PERKESO : <strong>{employee.socsoNumber || '-'}</strong></span>
              </div>
              <div style={{ display: 'flex', gap: '40px', marginBottom: '3px' }}>
                <span>Nama : <strong>{employee.fullName}</strong></span>
                <span>Lokasi : <strong>{branch?.name || employee.branch || '-'}</strong></span>
              </div>
              <div style={{ display: 'flex', gap: '40px' }}>
                <span>Jawatan : <strong>{employee.position}</strong></span>
                <span>Bank &amp; AC No. : <strong>{employee.bankName ? `${employee.bankName} - ${employee.accountNumber}` : '-'}</strong></span>
              </div>
              <div style={{ display: 'flex', gap: '40px', marginTop: '3px' }}>
                <span>Project : <strong>{payroll.projectName || project?.name || '-'}</strong></span>
                <span>Pay Structure : <strong>{payStructure}</strong></span>
                <span>Normal OT : <strong>{payroll.normalOtMultiplier ? `${payroll.normalOtMultiplier}x` : '-'}</strong></span>
              </div>
            </div>
