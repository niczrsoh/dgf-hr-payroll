...
interface PayrollHistoryPreviewProps {
  records: Array<{
    month: string;
    employeeNo: string;
    employeeName: string;
...
export default function PayrollHistoryPreview({ records, onClose }: PayrollHistoryPreviewProps) {
  const handlePrint = () => {
...
<html>
<head>
  <title>Salary Payment History Report</title>
  <style>
    @page {
      margin: 20mm;
    }
...
<body>
  <div class="company-header">
    ...
    <div class="company-address">Salary Payment Processing Department</div>
  </div>

  <div class="document-title">SALARY PAYMENT HISTORY REPORT</div>
  ...
  <div class="document-info">
    ...
    <div class="info-row">
      ...
      <div><span class="info-label">Total Net Salary:</span> RM ${totalNet.toFixed(2)}</div>
    </div>
  </div>
  ...
  <table>
    <thead>
      <tr>
        ...
        <th class="text-right">Gross Salary (RM)</th>
        <th class="text-right">Deductions (RM)</th>
        <th class="text-right">Net Salary (RM)</th>
    ...
    <tbody>
      ${records.map((record, index) => `
        <tr>
          ...
          <td class="text-right">${(record.grossSalary || 0).toFixed(2)}</td>
          <td class="text-right">${(record.totalDeduction || 0).toFixed(2)}</td>
          <td class="text-right">${(record.netSalary || 0).toFixed(2)}</td>
    ...
    <tfoot>
      <tr style="background-color: #f5f5f5; font-weight: bold;">
        <td colspan="5" class="text-right" style="font-size: 10pt;">TOTALS:</td>
        <td class="text-right" style="font-size: 10pt; font-weight: bold;">RM ${totalGross.toFixed(2)}</td>
        <td class="text-right" style="font-size: 10pt; font-weight: bold; color: #dc2626;">RM ${totalDeduction.toFixed(2)}</td>
        <td class="text-right" style="font-size: 10pt; font-weight: bold;">RM ${totalNet.toFixed(2)}</td>
        <td colspan="2"></td>
      </tr>
    </tfoot>
  </table>
  ...
  <div class="summary-section">
    ...
    <div class="summary-row">
      ...
      <div class="summary-value">RM ${totalGross.toFixed(2)}</div>
    </div>
    <div class="summary-row">
      <div class="summary-label">Total Deductions:</div>
      <div class="summary-value" style="color: #dc2626;">RM ${totalDeduction.toFixed(2)}</div>
    </div>
    <div class="summary-row total-amount">
      <div class="summary-label">TOTAL NET SALARY:</div>
      <div class="summary-value">RM ${totalNet.toFixed(2)}</div>
    </div>
  </div>
  ...
  <div class="signature-section">
    <div class="signature-box">
      ...
      <div style="font-size: 9pt; margin-top: 5px;">Payroll Department</div>
    </div>
    <div class="signature-box">
  ...
  <div class="footer">
    ...
    <p>For any discrepancies, please contact the Payroll Department immediately.</p>
    <p style="margin-top: 10px;">Document Reference: SAL-${new Date().getFullYear()}${String(new Date().getMonth() + 1).padStart(2, '0')}-${String(new Date().getDate()).padStart(2, '0')}-${Date.now().toString().slice(-6)}</p>
  </div>
</body>
</html>
      `;
  ...
  const handleDownloadExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(
      records.map((record, index) => ({
        ...
        'Gross Salary (RM)': (record.grossSalary || 0).toFixed(2),
        'Deductions (RM)': (record.totalDeduction || 0).toFixed(2),
        'Net Salary (RM)': (record.netSalary || 0).toFixed(2),
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Salary Payment History');

    XLSX.writeFile(workbook, `salary-payment-history-${new Date().toISOString().split('T')[0]}.xlsx`);
    toast.success('Excel file downloaded');
  };

  const totalGross = records.reduce((sum, record) => sum + record.grossSalary, 0);
  const totalDeduction = records.reduce((sum, record) => sum + record.totalDeduction, 0);
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-xl max-w-7xl w-full max-h-[90vh] overflow-hidden flex flex-col">
        ...
        <div className="px-6 py-4 border-b border-slate-200 flex items-center justify-between bg-slate-50">
          <div>
            <h3 className="text-lg font-semibold text-slate-900">Salary Payment History Report</h3>
            <p className="text-sm text-slate-600 mt-1">
              Total Records: {records.length} • Total Net Salary: RM {totalNet.toFixed(2)}
            </p>
          </div>
          <button
            onClick={onClose}
            ...
            <table className="w-full text-sm border-collapse">
              <thead className="bg-slate-50 border-b-2 border-slate-300">
                <tr>
                  ...
                  <th className="px-4 py-3 text-right text-xs font-semibold text-slate-700 border border-slate-300">Gross Salary (RM)</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold text-slate-700 border border-slate-300">Deductions (RM)</th>
                  <th className="px-4 py-3 text-right text-xs font-semibold text-slate-700 border border-slate-300">Net Salary (RM)</th>
              ...
              <tbody className="divide-y divide-slate-200">
                {records.map((record, index) => (
                  <tr key={index} className="hover:bg-slate-50">
                    ...
                    <td className="px-4 py-3 text-right text-slate-900 border border-slate-300">
                      {(record.grossSalary || 0).toFixed(2)}
                    </td>
                    <td className="px-4 py-3 text-right text-red-600 border border-slate-300">
                      {(record.totalDeduction || 0).toFixed(2)}
                    </td>
                    <td className="px-4 py-3 text-right font-semibold text-green-600 border border-slate-300">
              ...
              <tfoot className="bg-slate-50 border-t-2 border-slate-300">
                <tr>
                  <td colSpan={5} className="px-4 py-3 text-right font-bold text-slate-900 border border-slate-300">
                    Total:
                  </td>
                  <td className="px-4 py-3 text-right font-bold text-slate-900 border border-slate-300">
                    RM {totalGross.toFixed(2)}
                  </td>
                  <td className="px-4 py-3 text-right font-bold text-red-600 border border-slate-300">
